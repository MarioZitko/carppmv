...
from app.wikipedia.browse import MAX_ROWS, EngineRow, clean_query, rank_engine_rows
from app.wikipedia.schemas import WikipediaEngineRow, WikipediaEngineSearchResponse
...
async def search_engines(
    ...
    """Every engine row for a brand, optionally filtered by free text.

    Rows with no CO2 at all are excluded: a null CO2 is a valid extraction
    (plan §0) but this endpoint exists to show CO2 values, and a list of blanks
    ...
    # \"Attraction\""), and every one of those noise tokens would have to match
    # a row token for anything to come back — so the raw blob returns nothing at
    # all. Cleaned to "a3 1 2 tfsi" it returns the A3 engines. A deliberate
    # short query like "1.4 TDI" is unaffected: cleaning is a no-op on it.
    cleaned = clean_query(q, brand=canonical)
    ranked = rank_engine_rows([EngineRow.from_row(r) for r in rows], cleaned, limit=limit)

    return WikipediaEngineSearchResponse(
        brand=canonical,
        brand_known=bool(rows),
        rows=[WikipediaEngineRow(**vars(row)) for row in ranked],
    )
